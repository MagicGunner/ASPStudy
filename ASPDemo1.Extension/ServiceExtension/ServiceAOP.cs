...
public class ServiceAOP : IInterceptor {
    ...
    public void Intercept(IInvocation invocation) {
        ...
        try {
            ...
            json = "无法序列化，可能是兰姆达表达式等原因造成，按照框架优化代码" + ex;
        }

        var startTime = DateTime.Now;
        var apiLogAopInfo = new AOPLogInfo {
            ...
            if (IsAsyncMethod(invocation.Method)) {
                ...
                if (invocation.Method.ReturnType == typeof(Task))
                    invocation.ReturnValue = InternalAsyncHelper.AwaitTaskWithPostActionAndFinally((Task)invocation.ReturnValue,
                                                                                                   async () => await SuccessAction(invocation, apiLogAopInfo, startTime), /*成功时执行*/ ex => {
                ...
                try {
                    ...
                    jsonResult = "无法序列化，可能是兰姆达表达式等原因造成，按照框架优化代码" + ex;
                }

                var endTime = DateTime.Now;
                var responseTime = (endTime - startTime).Milliseconds.ToString();
    ...
    private void LogEx(Exception? ex, AOPLogInfo dataIntercept) {
        if (ex != null) Console.WriteLine("error!!!:" + ex.Message + JsonConvert.SerializeObject(dataIntercept));
    }
...
internal static class InternalAsyncHelper {
    ...
    public static object CallAwaitTaskWithPostActionAndFinallyAndGetResult(Type                taskReturnType,
        ...
        return typeof(InternalAsyncHelper)
              ...
              .Invoke(null, [
                                actualReturnValue, action, finalAction
                            ]);
    }
}